...
public class Constants {
    ...
    //xxPatch这个结尾的类，这个是补丁类，包含了修复bug的全部逻辑代码，这部分代码比较多，
    // 这个包含的主要代码就是对改动类的一次翻译：把改动方法中调用的方法和字段访问全部改为了反射调用，同时要解决Proguard造成的混淆、以及内联的问题
    public static final String PATCH_SUFFIX = "Patch";
    // PatchControl结尾的类被我们成为转发器，这个类的主要职责就是把APK中的某些方法转到补丁中的对应方法
    ...
    public static final Boolean OBSCURE = true; //混淆开关
