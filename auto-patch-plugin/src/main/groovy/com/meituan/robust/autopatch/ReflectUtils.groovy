        ...
        if (isStatic) {
            if (AccessFlag.isPublic(field.modifiers)) {
                ...
                if (field.declaringClass.name.equals(patchClassName)) {
                    ...
                    $_=($r)com.meituan.robust.utils.EnhancedRobustUtils.getStaticFieldValue(field混淆字段名,修改类名.class)
                     */
                    ...
                    $_=($r)com.meituan.robust.utils.EnhancedRobustUtils.getStaticFieldValue(field混淆字段名,field所在类的class文件)
                     */
            ...
            非static $0 表示 this
                ...
                if(this instanceof xxInLinePatch){
                    instance = ((xxxInLinePatch)this).originClass;
                }else{
                    instance = this;
                }
             */
            ...
            $_  表示返回值
            $r  返回结果的类型，用于强制类型转换
            getMappingValue(field.name, memberMappingInfo) 得到field混淆的 字段名

            $_ = ($r)com.meituan.robust.utils.EnhancedRobustUtils.getFieldValue(field混淆字段名,instance,field所在类class文件);
             */
...
//        println field.getName() + "  get field repalce  by  " + stringBuilder.toString() + "\n"
    ...
    static String getParameterClassSignure(String signature, String pacthClassName) {
        ...
        //signature 是 (Ljava/lang/String;)V 或者 (I)V 或者  (Lcom/javassist/JavassistLearn2;Lcom/javassist/JavassistLearn3;Ljava/lang/String;)V 三个参数的构造函数
        // (Lcom/javassist/JavassistLearn2;Lcom/javassist/JavassistLearn3;Ljava/lang/String;[I)V 原型  public JavassistLearn(JavassistLearn2 jl2,JavassistLearn3 jl3,String key,int[] Ints) {}
        // ([ILcom/javassist/JavassistLearn2;Lcom/javassist/JavassistLearn3;Ljava/lang/String;)V  原型 public JavassistLearn(int[] Ints,JavassistLearn2 jl2,JavassistLearn3 jl3,String key) {}

        //index = 1 定位到L  表示 全类引用 以L开头、;结尾，中间是引用类型的全类名
        for (int index = 1; index < signature.indexOf(")"); index++) {

            //OBJECT_TYPE 是 L    PACKNAME_END 是 ;
            if (Constants.OBJECT_TYPE == signature.charAt(index) && signature.indexOf(Constants.PACKNAME_END) != -1) {
                ...
                if (name.equals(pacthClassName)) {
                    //getmodifiedClassName(xxInLinePath) 从xxInLinePath 得到 xx 类
...
//        println("getMethodCallString  " + stringBuilder.toString())
    ...
    static String getInLineMemberString(CtMethod method, boolean isInStaticMethod, boolean isNewClass) {
        ...
        if (NameManger.getInstance().getPatchNameMap().get(NameManger.getInstance().getInlinePatchNameWithoutRecord(method.declaringClass.name)) != null) {
            ...
            //申明变量  SecondActivityInLinePatch instance;
            ...
            if (isInStaticMethod || isNewClass) {
                ...
                //Constants.GET_REAL_PARAMETER 是 getRealParameter
                ...
                if (!isStatic(method.modifiers)) {
                    if (signatureBuilder.toString().length() > 1) {
                        //Constants.ROBUST_UTILS_FULL_NAME 是 com.meituan.robust.utils.EnhancedRobustUtils
