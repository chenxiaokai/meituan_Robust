...
class PatchesFactory {
    ...
    /*
     * @param modifiedClass  内联类的CtClass对象
     * @param isInline  是否内联 true
     * @param patchName   例如返回 com.meituan.robust.patch+"."+SecondActivity+InLinePatch 以InLinePatch的后缀
     * @param patchMethodSignureSet
     *          内联类CtClass中的所有方法签名(就是modifiedClass中所有被消除的内联方法)
     *          methods need patch,if patchMethodSignatureSet length is 0,then will patch all methods in modifiedClass
     * @param patchPatch  生成patch的目录 patchPath = app/build/outputs/robust/
     * @return
     */
    private CtClass createPatchClass(CtClass modifiedClass, boolean isInline, String patchName, Set patchMethodSignureSet, String patchPath) throws CannotCompileException, IOException, NotFoundException {
        ...
        if (patchMethodSignureSet.size() != 0) {
            for (CtMethod method : modifiedClass.getDeclaredMethods()) {
                ...
                if ((!patchMethodSignureSet.contains(method.getLongName()) ||
                    ...
                    //methodNoNeedPatchList 是 modifiedClass不是内联方法集合
                    methodNoNeedPatchList.add(method);
                } else {
                    //移除methodNeedPatchSet中需要补丁的方法，留在补丁类中的方法默认全部会被处理
                    Config.methodNeedPatchSet.remove(method.getLongName()); //移除 有@Modify注解方法 和 RobustModify.modify() 调用方法 又是 内联方法
                }
            }
        }
        ...
        //处理super方法的调用
        //temPatchClass 是 SecondActivityInLinePatch，modifiedClass 内联类譬如SecondActivity， patchPath = app/build/outputs/robust/
                ...
                method.instrument(
                        new ExprEditor() {
                            public void edit(FieldAccess f) throws CannotCompileException {
                                //遍历方法每一行，如果有属性调用就会回调这个方法，f就是当前访问的属性字段
                                if (Config.newlyAddedClassNameList.contains(f.getClassName())) { //f.getClassName() 返回字段所在的类
                                    return;
                                }
                                //f.getField().declaringClass.name 得到字段 所在的类
                            ...
                            void edit(NewExpr e) throws CannotCompileException {
                                ...
                                try {
                                    ...
                                    if (!ReflectUtils.isStatic(Config.classPool.get(e.getClassName()).getModifiers()) && JavaUtils.isInnerClassInModifiedClass(e.getClassName(), modifiedClass)) {
                                        /*
                                        ReflectUtils.getNewInnerClassString方法参数
                                             1)构造方法签名 e.getSignature()  (Ljava/lang/String;)V
                                             2)temPatchClass.getName()  xxxInLinePath
                                             3)new 的这个类 是否为静态
                                             4)getClassValue 返回类的混淆后的类名，譬如android.support.design.R$attr -> android.support.design.a$b: 返回就是android.support.design.a$b
                                         */
                                        //代替new方法调用，改为反射调用
...
//                                        if (m.isSuper()) {
    ...
    //创建编译后apk擦除的内联类
    //sourceClass 是 SecondActivity  patchName 是 com.meituan.robust.patch.SecondActivityInLinePatch
    public
    static CtClass cloneClass(CtClass sourceClass, String patchName, List<CtMethod> exceptMethodList) throws CannotCompileException, NotFoundException {
        ...
        for (CtMethod method : sourceClass.getDeclaredMethods()) {
            if (null == exceptMethodList || !exceptMethodList.contains(method)) {
                //这个 if 条件就是 增加 targetClass 的内联方法
    ...
    //patchClass 是 SecondActivityInLinePatch，modifiedClass 内联类譬如SecondActivity， patchPath = app/build/outputs/robust/
    private void dealWithSuperMethod(CtClass patchClass, CtClass modifiedClass, String patchPath) throws NotFoundException, CannotCompileException, IOException {
        ...
        for (int index = 0; index < invokeSuperMethodList.size(); index++) {
            ...
            if (invokeSuperMethodList.get(index).getParameterTypes().length > 0) {
                /*
                var0,var1,var2 是CtMethod 的方法参数
                public static [returnType] staticRobust[methodName](xxxInLinePath patchInstance, xxx modifiedInstance,var0,var1,var2..){
                 */
                ...
                /*
                 public static [returnType] staticRobust[methodName](xxxInLinePath patchInstance, xxx modifiedInstance){
                 */
                methodBuilder.append("public  static  " + invokeSuperMethodList.get(index).getReturnType().getName() + "  " + ReflectUtils.getStaticSuperMethodName(invokeSuperMethodList.get(index).getName())
